...
"""
This library was modified by YSuzuki from original version served at the site above.
"""
...
def get_definition(candidate, sentence):
    ...
    ''' logging.debug("Checking definition of {} from '{}'".format(candidate,str(tokens))) '''
...
def extract_abbreviation_definition_pairs(file_path=None, doc_text=None, tagged_text=None):
    abbrev_map = dict()
    omit = 0
    written = 0
    mode=None
    if file_path:
        ...
        sentence_iterator = list(y(tagged_text))
        mode='awesome'
    else:
        return abbrev_map
            ...
            for candidate in best_candidates(sentence):
                try:
                    ...
                    try:
                        ...
                        if mode=='awesome':
                            abbrev_map[candidate] = (i,definition)
                        else:
